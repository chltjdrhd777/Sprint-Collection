...
buttons.addEventListener('click', function (event) {
  ...
  if (target.matches('button')) {
    ...
    if (action === 'number') {
      ...
      // 아래 코드가 작동됩니다.
      if(firstOperend.textContent === '0'){ //아직 입력이 안되었다.
        firstOperend.textContent = buttonContent; //첫번째 숫자가 입력이 안되었으니 
      } else {
    ...
    if (action === 'clear') { // 다같이 초기화
      //console.log('초기화 버튼');
      firstOperend.textContent = 0;
      secondOperend.textContent = 0;
      operator.textContent = "+"; //기본 모드가 더하기였으니까.
...
buttons.addEventListener('click', function (event) {
  ...
  if (target.matches('button')) {
    if (action === 'number') {
      ...
      } else { // 0아닌 숫자가 앞에 있을 경우
        display.textContent = display.textContent.concat(buttonContent);
      } 
    } previousKey = 'number';
    
    if (action === 'operator') { 
      operatorForAdvanced = buttonContent; // 연산자 버튼 누른거를 저장
      firstNum = display.textContent;
      previousKey = 'operator'; 
      //previousKey = 'operator';
      //previousKey = buttonContent;
     // if(previousKey ==="operator"){
       // display.textContent = buttonContent[1];//[1]:1번째 값부터 출력 
      //}
      //else 
      //display.textContent = display.textContent.concat(buttonContent);
    }
