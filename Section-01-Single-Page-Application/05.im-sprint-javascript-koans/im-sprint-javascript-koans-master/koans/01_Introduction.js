describe('expect에 대해서 학습합니다.', function () {
  ...
  */
  ...
  it('테스트하는 값(expect의 인자)이 falsy 여부를 검사합니다.', function () {
    ...
    expect(false).to.be.false; // 보충 : expect 값이 false인지 판단하게
  });
  ...
  */
  it("'테스트하는 값'을 '기대하는 값'과 비교한 결과가 참 인지 확인합니다.", function () {
    ...
    let actualValue = 1 + 1;
    let expectedValue = 2; // FILL_ME_IN; // TODO: 'FILL_ME_IN'을 변경하여 테스트 케이스를 완성합니다.
    expect(actualValue === expectedValue).to.be.true; //보충 : 같다고 나와야하니까 1+1과 같은 2가 있어야.
  });
  ...
  */
  it('Matcher .equal 의 사용법을 학습합니다.', function () {
    let expectedValue = 2; // FILL_ME_IN; // TODO
    // .equal은 두 값이 타입까지 엄격하게 같은지 검사(strict equlity, ===)합니다.
    expect(1 + 1).to.equal(expectedValue); // 보충 : to be true === .equal
  });

  it('Matcher .equal의 사용법을 학습합니다.', function () {
    let actualValue = (1 + 1).toString();
    expect(actualValue).to.equal('2'); // 보충 : 문자열 2와 같아야하니까.
  });
});
