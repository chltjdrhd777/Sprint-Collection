describe('type에 대해서 학습합니다.', function () {
  it("비교연산자 '=='는 두 값의 일치 여부를 느슨하게 검사(loose equality)합니다.", function () {
    let actualValue = 1 + 1;
    let expectedValue = '2'// FILL_ME_IN 느슨한 검사를 해주는 거 확인
  ...
  */
  ...
  it("비교연산자 '==='는 두 값의 일치 여부를 엄격하게 검사(strict equality)합니다.", function () {
    let actualValue = 1 + 1;
    let expectedValue = 2; //FILL_ME_IN; 앞에 1번 참조 바람.
    expect(actualValue === expectedValue).to.be.true;
  ...
  it('expect의 인자로 들어간 표현식의 평가(evaluation) 결과를 예측해 봅니다.', function () {
    expect(123 - '1').to.equal(122); // 보충 : 문자열 빼기는 숫자빼기로 인식
  });

  it('expect의 인자로 들어간 표현식의 평가(evaluation) 결과를 예측해 봅니다.', function () {
    expect(1 + true).to.equal(2); // 보충 : false : 0, true : 1로 인식 2진법.
  });

  it('expect의 인자로 들어간 표현식의 평가(evaluation) 결과를 예측해 봅니다.', function () {
    expect('1' + true).to.equal('1true'); // 스트링 더해지니까
  });
