 ...
 */
  it('원시 자료형은 값 자체에 대한 변경이 불가능(immutable)합니다.', function () {
    let name = 'codestates';
    expect(name).to.equal('codestates'); //(FILL_ME_IN);
    expect(name.toUpperCase()).to.equal('CODESTATES'); //(FILL_ME_IN);
    expect(name).to.equal('codestates'); //(FILL_ME_IN);
    // 새로운 값으로 재할당은 가능합니다.
  ...
  it('원시 자료형을 변수에 할당할 경우, 값 자체의 복사가 일어납니다.', function () {
    let overTwenty = true;
    let allowedToDrink = overTwenty; // 위의 값을 할당한거니까 

    overTwenty = false;
    expect(overTwenty).to.equal(false);//(FILL_ME_IN);
    expect(allowedToDrink).to.equal(true);//(FILL_ME_IN);

    let variable = 'variable';
    let variableCopy = 'variableCopy';
    variableCopy = variable;
    variable = variableCopy;
    expect(variable).to.equal('variable'); //(FILL_ME_IN);
  });
  ...
  it('원시 자료형 또는 원시 자료형의 데이터를 함수의 인자로 전달할 경우, 값 자체의 복사가 일어납니다.', function () {
    ...
    expect(currentYear).to.equal(2020);//(FILL_ME_IN);
    ...
    expect(currentYear).to.equal(2020); // 안쪽 스코프에 선인이 된 것이 없어서 2020!
    expect(after10).to.equal(2030); //(FILL_ME_IN);
  ...
  */
  it('참조 자료형의 데이터는 동적(dynamic)으로 변합니다.', function () {
    const arr = [1, 2, 3];
    expect(arr.length).to.equal(3);//(FILL_ME_IN);
    arr.push(4, 5, 6);
    expect(arr.length).to.equal(6);//(FILL_ME_IN);
    arr.pop();
    expect(arr.length).to.equal(5);//(FILL_ME_IN);

    const obj = {};
    expect(Object.keys(obj).length).to.equal(0);//(FILL_ME_IN);
    obj['name'] = 'codestates';
    obj.quality = 'best';
    obj.product = ['sw engineering', 'product manager', 'growth marketing', 'data science'];
    expect(Object.keys(obj).length).to.equal(3);//(FILL_ME_IN);
    delete obj.name;
    expect(Object.keys(obj).length).to.equal(2);//(FILL_ME_IN);
  });
  ...
  it('참조 자료형을 변수에 할당할 경우, 데이터의 주소가 저장됩니다.', function () {
    ...
    */
    ...
    overTwenty.push('san');
    expect(allowedToDrink).to.deep.equal(['hongsik', 'minchul', 'hoyong','san']);//(FILL_ME_IN);
    overTwenty[1] = 'chanyoung';
    expect(allowedToDrink[1]).to.deep.equal('chanyoung');
    // 배열의 요소가 다 같이 찬영으로 바뀜
    ...
    expect(allowedToDrink === [22, 23, 27]).to.equal(false); // 주소값 달라서 틀림

    const nums1 = [1, 2, 3];
    const nums2 = [1, 2, 3];
    expect(nums1 === nums2).to.equal(false); // heap의 주소값 달라서 틀림
    ...
    expect(person.son === boy).to.equal(true); // 같음
