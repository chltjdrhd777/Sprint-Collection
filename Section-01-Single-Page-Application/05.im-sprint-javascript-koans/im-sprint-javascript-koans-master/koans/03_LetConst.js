describe("'const'에 대해서 학습합니다.", function () {
  it("'const'로 선언된 변수에는 재할당(reassignment)이 금지됩니다.", function () {
    ...
    // constString = "which means I'm a constant variable, delete me."; (재할당 추측)
    // const 변수 선언이 딱 한 번만! 오류케이스가 바로 출력이됨.
    expect(constString).to.equal('I am a const');
  });

  it("'const'로 선언된 배열의 경우 새로운 요소를 추가하거나 삭제할 수 있습니다.", function () {
    const arr = [];
    const toBePushed = 42; //보충: 42를 추가 //FILL_ME_IN;
    arr.push(toBePushed);
  ...
  it("'const'로 선언된 객체의 경우, 속성을 추가하거나 삭제할 수 있습니다.", function () {
    const obj = { x: 1 };
    expect(obj.x).to.equal(1); // 요소 물어보는

    delete obj.x;
    expect(obj.x).to.equal(undefined); // 정의 안되서
