describe('Object에 대해서 학습합니다.', function () {
  ...
  */
  it('Object의 기본을 확인합니다.', function () {
    const emptyObj = {};
    expect(typeof emptyObj === 'object').to.equal(true);//(FILL_ME_IN);
    expect(emptyObj.length).to.equal(undefined);//(FILL_ME_IN);
    ...
    expect(megalomaniac.length).to.equal(undefined); // 객체 키 이름으로 오인 - Object.keys() 길이다
  ...
  it('Object의 속성(property)를 다루는 방법을 확인합니다.', function () {
    ...
    expect('mastermind' in megalomaniac).to.equal(true); // 그 요소가 있는지를 확인하는 문장
  ...
  it("'this'는 method를 호출하는 시점에 결정됩니다.", function () {
    const currentYear = new Date().getFullYear(); //new Date() 현재 날짜, getFullYear() 년도 // 현재연도
    const megalomaniac = {
  ...
  it('Object를 함수의 인자로 전달할 경우, reference가 전달됩니다.', function () {
    ...
    expect(obj.henchwoman).to.equal('Adam West'); // 아담웨스트인게 헨치우먼을 바꿔서 그런거같음.

    const assignedObj = obj; // 카피를 만듦
    assignedObj['relations'] = [1, 2, 3]; // 카피본에 릴레이션을 바꿈
    ...
    expect(copiedObj.henchwoman).to.equal('Adam West'); // 이거랑 별개인 카피드 오브젝트를 물어본거잖아.
