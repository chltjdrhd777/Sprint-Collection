describe('Spread syntax에 대해 학습합니다.', function () {
  ...
  it('Rest Parameter는 함수의 인자를 배열로 다룰 수 있게 합니다.', function () {
    ...
    expect(Object.keys(argumentsObj)).to.deep.equal([ '0', '1', '2' ]); // 인덱스 번호 토해냄
    ...
    const argsArr = Array.from(argumentsObj); // 원본 복사와 비슷한 개념
    expect(Array.isArray(argsArr)).to.deep.equal(true);
    expect(argsArr).to.deep.equal([ 'first', 'second', 'third' ]);
    expect(argsArr === restParams).to.deep.equal(false); // 생긴거만 배열 객체로 인식해옴
  });
  ...
  it('Rest Parameter는 인자의 일부에만 적용할 수도 있습니다.', function () {
    ...
    expect(getAllParams(123)).to.deep.equal([ 123, undefined, [] ] );//뒤에 값이 없어도 다 써야함
    ...
    expect(makePizza('original')).to.equal('You ordered undefined pizza with original dough and 0 extra toppings!');//(FILL_ME_IN);
    expect(makePizza('thin', 'pepperoni')).to.equal('You ordered pepperoni pizza with thin dough and 0 extra toppings!');//(FILL_ME_IN);
    expect(makePizza('napoli', 'meat', 'extra cheese', 'onion', 'bacon')).to.equal('You ordered meat pizza with napoli dough and 3 extra toppings!');//(FILL_ME_IN);
  });
});
