...
function createTreeView(menu, currentNode) {
  ...
  for(let i = 0; i < menu.length; i++){
    ...
    if(Object.keys(menu[i]).includes('children') === true){
...
/*
menu 데이터를 사용해 tree view를 구성합니다
  1) ul#root 엘리먼트 안에 카테고리(음료, 음식, 굿즈, 카드)를 렌더링할 4개의 li 엘리먼트가 있어야 합니다 @

  2) 음료, 음식, 굿즈, 카드 카테고리 이름(name)을 span 태그로 감싸야 합니다

  3) 자식 노드가 없는 데이터의 경우, li 엘리먼트 안에 단순히 이름(name)만 표시합니다. (checkbox 및 span, ul을 포함하면 안됩니다)
  
  4) 자식 노드가 있는 데이터의 경우, li 엘리먼트 아래에 자식 노드를 렌더링할 새로운 ul이 존재해야 합니다 (1)
  5) 자식 노드가 있는 데이터의 경우, li 엘리먼트 아래에 자식 노드를 렌더링할 새로운 ul이 존재해야 합니다 (2)

  6) type 속성이 item인 객체는 li 태그로 name 속성값을 감싸주어야 합니다
*/